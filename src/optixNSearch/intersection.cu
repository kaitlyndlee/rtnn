    ...
    for (int q = 0; q < state.numQueries; q++) {
      for (int p = 0; p < state.numPoints; p++) {
        ...
        for (int d = 0; d < state.dim / 3; d++) {
          ...
          totalSum += state.distances[d][get3DIndex(b, q, p, state.numQueries, state.numPoints)];
          // printf("Query %u, Point %u, Index, %u, Squared distance: %f\n", q, p, p, state.distances[d][get3DIndex(b, q, p, state.numQueries, state.numPoints)]);
        }
        if (totalSum < state.radius * state.radius) {
          // printf("Distance: %f\n", sqrt(totalSum[q]));
          totalDist += sqrt(totalSum);
        }
      }
    }
  }
