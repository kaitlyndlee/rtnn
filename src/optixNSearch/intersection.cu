  ...
  for (int b = 0; b < state.numOfBatches; b++) {
    for (int q = 0; q < state.numQueries; q++) {
      for (int p = 0; p < state.params.limit; p++) {
        ...
        totalSum += sqrt(state.distances[get3DIndex(b, q, index, state.numQueries, state.numPoints)]);
        printf("Query %u, Point %u, Index, %u, Squared distance: %f\n", q, p, index, state.distances[get3DIndex(b, q, index, state.numQueries, state.numPoints)]);
  }
    }
  }
  return totalSum;
