...
void fillByValue(thrust::device_ptr<float>, uint64_t, float);
float reduce(thrust::device_ptr<float>, uint64_t);
// thrust::device_ptr<float> remove(thrust::device_ptr<float>, uint64_t, float);
...
void launchSubframe( unsigned int*, RTNNState&, int, unsigned int, float3 *);
...
void reduceData(RTNNState &);
void allocateDistances(RTNNState &);
void allocResultArray(RTNNState &);
...
double bruteForceSearch(float3 **, float3 **, const double, const int, const int, const int, const int);
void allocBatches(RTNNState);
float calcMemUsage(RTNNState&);
