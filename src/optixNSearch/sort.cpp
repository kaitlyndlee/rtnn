...
void computeMinMax(unsigned int N, float3 *particles, float3 &min,
                   float3 &max) {
  // TODO: maybe use long since we are going to convert a float to its floor
  // value?
  thrust::host_vector<int3> h_MinMax(2);
  h_MinMax[0] = make_int3(std::numeric_limits<int>().max(),
                          std::numeric_limits<int>().max(),
                          std::numeric_limits<int>().max());
  h_MinMax[1] = make_int3(std::numeric_limits<int>().min(),
  ...
  kComputeMinMax(numOfBlocks, threadsPerBlock, particles, N,
                 thrust::raw_pointer_cast(&d_MinMax[0]),
                 thrust::raw_pointer_cast(&d_MinMax[1]));

  h_MinMax = d_MinMax;

  // minCell encloses the scene but maxCell doesn't (floor and int in the
  // kernel) so increment by 1 to enclose the scene.
  // TODO: consider minus 1 for minCell too to avoid the numerical precision
  // issue
  ...
  // minCell.y, minCell.z, maxCell.x, maxCell.y, maxCell.z);
...
unsigned int genGridInfo(RTNNState &state, unsigned int N, GridInfo &gridInfo) {
  ...
  gridInfo.GridDimension.x =
      static_cast<unsigned int>(ceilf(gridSize.x / cellSize));
  gridInfo.GridDimension.y =
      static_cast<unsigned int>(ceilf(gridSize.y / cellSize));
  gridInfo.GridDimension.z =
      static_cast<unsigned int>(ceilf(gridSize.z / cellSize));
  ...
  //   the strategy is to divide the grid into smaller
  //   equal-dimension-power-of-2 smaller grids (meta_grid here). the order
  //   within each meta_grid is morton, but the order across meta_grids is
  //   raster order.
  ...
  unsigned int shortestSide =
      std::min({gridInfo.GridDimension.x, gridInfo.GridDimension.y,
                gridInfo.GridDimension.z});
  // dim should at least be 1; otherwise we won't get 0 cells.
  gridInfo.meta_grid_dim =
      std::max((int)pow(2, floorf(log2(shortestSide))) / state.mcScale, 1);
  gridInfo.meta_grid_size =
      gridInfo.meta_grid_dim * gridInfo.meta_grid_dim * gridInfo.meta_grid_dim;
  ...
  gridInfo.MetaGridDimension.x = static_cast<unsigned int>(
      ceilf(gridInfo.GridDimension.x / (float)gridInfo.meta_grid_dim));
  gridInfo.MetaGridDimension.y = static_cast<unsigned int>(
      ceilf(gridInfo.GridDimension.y / (float)gridInfo.meta_grid_dim));
  gridInfo.MetaGridDimension.z = static_cast<unsigned int>(
      ceilf(gridInfo.GridDimension.z / (float)gridInfo.meta_grid_dim));

  // metagrids will slightly increase the total cells
  unsigned int numberOfCells =
      (gridInfo.MetaGridDimension.x * gridInfo.MetaGridDimension.y *
       gridInfo.MetaGridDimension.z) *
      gridInfo.meta_grid_size;
  fprintf(stdout, "\tGrid dimension (without meta grids): %u, %u, %u\n",
          gridInfo.GridDimension.x, gridInfo.GridDimension.y,
          gridInfo.GridDimension.z);
  fprintf(stdout, "\tGrid dimension (with meta grids): %u, %u, %u\n",
          gridInfo.MetaGridDimension.x * gridInfo.meta_grid_dim,
          gridInfo.MetaGridDimension.y * gridInfo.meta_grid_dim,
          gridInfo.MetaGridDimension.z * gridInfo.meta_grid_dim);
  // fprintf(stdout, "\tMeta Grid dimension: %u, %u, %u\n",
  // gridInfo.MetaGridDimension.x, gridInfo.MetaGridDimension.y,
  // gridInfo.MetaGridDimension.z); fprintf(stdout, "\t# of cells in a meta
  // grid: %u\n", gridInfo.meta_grid_dim); fprintf(stdout, "\tGridDelta: %f, %f,
  // %f\n", gridInfo.GridDelta.x, gridInfo.GridDelta.y, gridInfo.GridDelta.z);
  fprintf(stdout, "\tNumber of cells: %u\n", numberOfCells);
  fprintf(stdout, "\tCell size: %f\n", cellSize);

  // update GridDimension so that it can be used in the kernels (otherwise
  // raster order is incorrect)
  gridInfo.GridDimension.x =
      gridInfo.MetaGridDimension.x * gridInfo.meta_grid_dim;
  gridInfo.GridDimension.y =
      gridInfo.MetaGridDimension.y * gridInfo.meta_grid_dim;
...
genCellMask(RTNNState &state, unsigned int *d_repQueries, float3 *particles,
            unsigned int *d_CellParticleCounts, unsigned int numberOfCells,
            GridInfo gridInfo, unsigned int N, unsigned int numUniqQs,
            bool morton) {
  float cellSize = state.radius / state.crRatio;
  ...
  // CUDA_CHECK( cudaMemset ( thrust::raw_pointer_cast(d_cellMask), 0xFF,
  // numberOfCells * sizeof(int) ) );
  ...
  if (gpu) {
    ...
    // thrust::copy(thrust::device_pointer_cast(d_CellParticleCounts),
    // thrust::device_pointer_cast(d_CellParticleCounts) + numberOfCells,
    // h_CellParticleCounts.begin());

    unsigned int threadsPerBlock = 64;
    unsigned int numOfBlocks = numUniqQs / threadsPerBlock + 1;
    kCalcSearchSize(numOfBlocks, threadsPerBlock, gridInfo, morton,
                    d_CellParticleCounts, d_repQueries, particles, cellSize,
                    maxWidth, state.knn, thrust::raw_pointer_cast(d_cellMask));

    // thrust::host_vector<int> h_cellMask_t(numberOfCells);
    // thrust::copy(d_cellMask, d_cellMask + numberOfCells,
    // h_cellMask_t.begin());
  } else {
    thrust::host_vector<unsigned int> h_part_seq(numUniqQs);
    thrust::copy(thrust::device_pointer_cast(d_repQueries),
                 thrust::device_pointer_cast(d_repQueries) + numUniqQs,
                 h_part_seq.begin());

    thrust::host_vector<unsigned int> h_CellParticleCounts(numberOfCells);
    thrust::copy(thrust::device_pointer_cast(d_CellParticleCounts),
                 thrust::device_pointer_cast(d_CellParticleCounts) +
                     numberOfCells,
                 h_CellParticleCounts.begin());
    ...
    for (unsigned int i = 0; i < numUniqQs; i++) {
      ...
      int3 gridCell =
          make_int3(int(gridCellF.x), int(gridCellF.y), int(gridCellF.z));

      calcSearchSize(gridCell, gridInfo, morton, h_CellParticleCounts.data(),
                     cellSize, maxWidth, state.knn, h_cellMask.data());
    }
...
void autoBatchingRange(RTNNState &state,
                       const thrust::host_vector<unsigned int> &h_rayHist,
                       std::vector<int> &batches, int numAvailBatches) {
  ...
  // const float kAABBTest_PerIS = 4e-5/50; // IS call time in *ms* if doing
  // aabb test (by 50 as the data was ubenchmarked using K=50) const float
  // kSphereTest_PerIS = 4e-4/50; // IS call time in *ms* if doing sphere test

  // empirical coefficients on 2080
  const float kAABBTest_PerIS = 1e-5 / 50;
  const float kSphereTest_PerIS = 1e-4 / 50;

  // float tMemcpy = state.numQueries * state.knn * sizeof(unsigned int) *
  // kD2H_PerB; // TODO: consider max(memcpy, compute)
  float tBuildGAS = state.numPoints * kBuildGas_PerAABB +
                    20; // 20 is the empirical intercept (TODO)
  ...
  for (int i = numAvailBatches - 2; i >= 0; i--) {
    float extraTime =
        h_rayHist[i] * (kSphereTest_PerIS - kAABBTest_PerIS) * state.knn;
    overhead += extraTime - tBuildGAS;
    // fprintf(stdout, "i: %d, %u extraTime: %f, overhead: %f\n", i,
    // h_rayHist[i], extraTime, overhead);
...
float radiusFromMegacell(float width, int approxMode) {
  if (approxMode == 2)
    return radiusEquiVolume(width, 3); // 0.62; works well for uniform density
  ...
  else if (approxMode == 1)
    return minCircumscribedRadius(width, 2); // 0.71
  else
    return minCircumscribedRadius(width, 3); // 0.87
}

void autoBatchingKNN(RTNNState &state,
                     const thrust::host_vector<unsigned int> &h_rayHist,
                     std::vector<int> &batches, int numAvailBatches) {
  ...
  // GAS building time is linear w.r.t. to the # of AABBs, which is the total
  // amount of points. Searching time = max(memcpy time, compute time). The
  // memcpy time is empirically observed to be linear w.r.t., to the # of
  // queries The compute time, without considering CKE, is the lump sum of the
  // compute time of each batch, which is linear w.r.t. the # of queries in the
  // batch and cubic w.r.t., to the radius in the batch.
  ...
  float maxRadius =
      std::min(state.radius, radiusFromMegacell(maxWidth, state.approxMode));
  // incrementally combine batch i with the last batch (assuming all other
  // batches are independent) and calculate the cost. choose the min cost.
  float overhead = 0;
  float maxOverhead = 0;
  ...
  for (int i = numAvailBatches - 2; i >= 0; i--) {
    ...
    float curRadius =
        std::min(state.radius, radiusFromMegacell(curWidth, state.approxMode));
    float density = state.knn / ((curWidth - cellSize) * (curWidth - cellSize) *
                                 (curWidth - cellSize));

    // TODO: assuming density doesn't change dramatically; consider non-uniform
    // density?
    float extraWork = h_rayHist[i] * 8 *
                      (maxRadius * maxRadius * maxRadius -
                       curRadius * curRadius * curRadius) *
                      density;
    float extraTime = extraWork * kSearch_PerIS;
    overhead += extraTime - tBuildGAS;
    // fprintf(stdout, "i: %d, density: %f, extraWork: %f, extraTime: %f,
...
void prepBatches(RTNNState &state, std::vector<int> &batches,
                 const thrust::host_vector<unsigned int> &h_rayHist) {
  ...
  if (state.autoNB) {
    if (state.searchMode == "knn")
      autoBatchingKNN(state, h_rayHist, batches, numAvailBatches);
    else
      autoBatchingRange(state, h_rayHist, batches, numAvailBatches);
  } else {
    ...
    int numBatches;
    if (state.numOfBatches == -1)
      numBatches = (int)numAvailBatches;
    else
      numBatches = std::min(state.numOfBatches, (int)numAvailBatches);

    for (int i = 0; i < numAvailBatches; i++) {
      if (i <= numBatches - 2 || i == numAvailBatches - 1)
        batches.push_back(i);
    }
...
void genBatches(RTNNState &state, std::vector<int> &batches,
                thrust::host_vector<unsigned int> h_rayHist, float3 *particles,
                unsigned int N, thrust::device_ptr<int> d_rayMask) {
  float cellSize = state.radius / state.crRatio;
  ...
  for (int batchId = 0; batchId < state.numOfBatches; batchId++) {
    ...
    float partThd = kGetWidthFromIter(
        maxMask, cellSize); // partThd depends on the max mask.
    if (state.searchMode == "knn")
      state.launchRadius[batchId] =
          radiusFromMegacell(partThd, state.approxMode);
    else
      state.launchRadius[batchId] = partThd / 2;
    if (batchId == (state.numOfBatches - 1))
      state.launchRadius[batchId] = state.radius;
...
void sortGenBatch(RTNNState &state, unsigned int N, bool morton,
                  unsigned int numberOfCells, unsigned int numOfBlocks,
                  unsigned int threadsPerBlock, GridInfo gridInfo,
                  float3 *particles,
  ...
  unsigned int numUniqQs =
      uniqueByKey(d_ParticleCellIndices_ptr_copy, N, d_repQueries);
  fprintf(stdout, "\tNum of Rep queries: %u\n", numUniqQs);

  // generate the cell mask
  thrust::device_ptr<int> d_cellMask =
      genCellMask(state, thrust::raw_pointer_cast(d_repQueries), particles,
                  thrust::raw_pointer_cast(d_CellParticleCounts_ptr),
                  numberOfCells, gridInfo, N, numUniqQs, morton);
  ...
  //   new grid.
  kCountingSortIndices_setRayMask(
      numOfBlocks, threadsPerBlock, gridInfo,
  ...
  // get a histogram of d_rayMask, which won't be mutated. this needs to happen
  // before sorting |d_rayMask|. the last mask in the histogram indicates the
  // number of rays that need full search.
  thrust::device_vector<unsigned int> d_rayHist;
  unsigned int numMasks =
      thrustGenHist(d_rayMask, d_rayHist,
                    N); // this would trigger an alloc and cudafree of size N
  ...
  if (state.querySortMode) {
    ...
    // TODO: Can we do away with the extra copy by replacing sort by key with
    // scatter? That'll need new space too...
    ...
    sortByKey(d_posInSortedPoints_ptr, thrust::device_pointer_cast(particles),
              N);
  }
...
void gridSort(RTNNState &state, unsigned int N, float3 *particles,
              float3 *h_particles, bool morton, ParticleType type) {
  ...
  thrust::device_ptr<unsigned int> d_CellParticleCounts_ptr =
      thrust::device_pointer_cast(
          reinterpret_cast<unsigned int *>(state.d_CellParticleCounts_ptr_p));
  // only need for sorting
  thrust::device_ptr<unsigned int> d_LocalSortedIndices_ptr;
  thrust::device_ptr<unsigned int> d_CellOffsets_ptr =
      thrust::device_pointer_cast(
          reinterpret_cast<unsigned int *>(state.d_CellOffsets_ptr_p));
  thrust::device_ptr<unsigned int> d_posInSortedPoints_ptr;
  ...
  if ((type == POINT) && state.partition) {
    ...
    allocThrustDevicePtr(&d_CellParticleCounts_ptr, numberOfCells,
                         &state.d_gridPointers);
  ...
  kInsertParticles(numOfBlocks, threadsPerBlock, gridInfo, particles,
                   thrust::raw_pointer_cast(d_ParticleCellIndices_ptr),
                   thrust::raw_pointer_cast(d_CellParticleCounts_ptr),
                   thrust::raw_pointer_cast(d_LocalSortedIndices_ptr), morton);

  fillByValue(d_CellOffsets_ptr, numberOfCells,
              0); // need to initialize it even for exclusive scan
  ...
  // thrust::copy(d_ParticleCellIndices_ptr, d_ParticleCellIndices_ptr+N,
  // test.begin()); for (unsigned int i = 0; i < test.size(); i++) {
  ...
  if (toPartition) {
    ...
    if (!state.sameData) {
      ...
      // inserting points. we don't need samepq to trigger this. sameData is
      // enough.
      ...
      kInsertParticles(
          numOfBlocks, threadsPerBlock, gridInfo, state.params.points, nullptr,
          thrust::raw_pointer_cast(d_CellParticleCounts_ptr_p), nullptr,
    ...
    state.d_CellParticleCounts_ptr_p =
        (void *)thrust::raw_pointer_cast(d_CellParticleCounts_ptr_p);
    state.d_CellOffsets_ptr_p =
        (void *)thrust::raw_pointer_cast(d_CellOffsets_ptr);

    sortGenBatch(state, N, morton, numberOfCells, numOfBlocks, threadsPerBlock,
                 gridInfo, particles, d_CellParticleCounts_ptr_p,
                 d_ParticleCellIndices_ptr, d_CellOffsets_ptr,
                 d_LocalSortedIndices_ptr, d_posInSortedPoints_ptr);
  } else {
    kCountingSortIndices(numOfBlocks, threadsPerBlock, gridInfo,
                         thrust::raw_pointer_cast(d_ParticleCellIndices_ptr),
                         thrust::raw_pointer_cast(d_CellOffsets_ptr),
                         thrust::raw_pointer_cast(d_LocalSortedIndices_ptr),
                         thrust::raw_pointer_cast(d_posInSortedPoints_ptr));
  ...
  if ((type == QUERY) && !state.samepq && (!state.sanCheck || toPartition))
    return;
  thrust::device_ptr<float3> d_particles_ptr =
      thrust::device_pointer_cast(particles);
  thrust::copy(d_particles_ptr, d_particles_ptr + N, h_particles);
}

void oneDSort(RTNNState &state, unsigned int N, float3 *particles,
              float3 *h_particles) {
  ...
  // note that the h_queries at this point still point to what h_points points
  // to
  thrust::copy(d_particles_ptr, d_particles_ptr + N, h_particles);
}
...
void sortParticles(RTNNState &state, ParticleType type, int sortMode) {
  ...
  if ((type == QUERY) && !state.partition && !sortMode)
    return;
  else if ((type == POINT) && !sortMode)
    return;
  ...
  // the semantices of sorting is: sort data in device, and copy the sorted data
  // back to host.
  if (sortMode == 3) {
...
sortQueriesByFHCoord(RTNNState &state,
                     thrust::device_ptr<unsigned int> d_firsthit_idx_ptr,
                     int batch_id) {
  // this is sorting queries by the x/y/z coordinate of the first hit
  // primitives.
  unsigned int numQueries = state.numActQueries[batch_id];

  Timing::startTiming("gas-sort queries init");
  // allocate device memory for storing the keys, which will be generated by a
  // gather and used in sort_by_keys
  thrust::device_ptr<float> d_key_ptr;
  ...
  thrust::device_vector<float> d_orig_points_1d =
      h_orig_points_1d; // one P NArray
  ...
  gatherByKey(d_firsthit_idx_ptr, &d_orig_points_1d, d_key_ptr, numQueries,
  ...
  if (debug) {
    ...
    for (unsigned int i = 0; i < h_vec_val.size(); i++) {
      std::cout << h_vec_key[i] << "\t" << h_vec_val[i] << "\t"
                << h_queries[h_vec_val[i]].x << "\t"
                << h_queries[h_vec_val[i]].y << "\t"
                << h_queries[h_vec_val[i]].z << std::endl;
    }
  }
...
thrust::device_ptr<unsigned int>
sortQueriesByFHIdx(RTNNState &state,
                   thrust::device_ptr<unsigned int> d_firsthit_idx_ptr,
                   int batch_id) {
  ...
  Timing::startTiming("gas-sort queries");
  sortByKey(d_firsthit_idx_ptr, d_r2q_map_ptr, numQueries,
            state.stream[batch_id]);
  ...
  if (debug) {
    ...
    thrust::copy(d_firsthit_idx_ptr, d_firsthit_idx_ptr + numQueries,
                 h_vec_key.begin());
...
void gatherQueries(RTNNState &state,
  ...
  thrust::device_ptr<float3> d_orig_queries_ptr =
      thrust::device_pointer_cast(state.d_actQs[batch_id]);

  // gather by key, which is generated by the previous sort
  gatherByKey(d_indices_ptr, d_orig_queries_ptr, d_reord_queries_ptr,
              numQueries, state.stream[batch_id]);
  ...
  if (state.sanCheck) {
    ...
    if (!state.samepq || state.partition)
      delete state.h_actQs[batch_id];
    state.h_actQs[batch_id] =
        new float3[numQueries]; // don't overwrite h_points
    thrust::copy(d_reord_queries_ptr, d_reord_queries_ptr + numQueries,
                 state.h_actQs[batch_id]);
  }
}
